...
class HomeView extends Component {
  ...
  render() {
    ...
    const displayJokes = jokes.length
      ? jokes.map(({ id, joke }) => {
          return <JokeElement key={id} text={joke} />;
        })
      : [];
    return (
      <div className="container">
        <div className="row">
          <div className="col-12 col-sm-6 col-lg-5 jokes-wrapper">
            {displayJokes}
          </div>

          <div className="col-12 col-sm-6 col-lg-5 offset-lg-5 jokes-wrapper"></div>
        </div>
      </div>
    );
  }
